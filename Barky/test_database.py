...
# Testing the delete function of the program, I added data then deleted it to make sure that it got deleted
def test_database_manager_delete_bookmark(database_manager):
    ...
    conn = database_manager.connection
    cursor = conn.cursor()
    cursor.execute(''' SELECT * FROM bookmarks WHERE url="http://example.com" ''')    
    # print(cursor.fetchone())
